          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {units.map(u => (
                <span key={u} className="px-3 py-1 bg-slate-100 text-slate-700 rounded-lg text-sm border border-slate-200 flex items-center gap-2">
                    {u} 
                    <button onClick={() => removeUnit(u)} className="text-slate-400 hover:text-rose-500">×</button>
                </span>
            ))}
            ...
            {!isAddingUnit ? (
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="w-20 px-2 py-1 text-sm border border-sky-300 rounded-lg outline-none"
                  placeholder="Unit"
                />
                <button onClick={addUnit} className="text-xs bg-sky-500 text-white px-2 py-1 rounded">Ox</button>
                <button onClick={() => setIsAddingUnit(false)} className="text-xs text-slate-400 px-1">Cancel</button>
              </div>
            )}
          </div>
          <p className="text-xs text-slate-400">These units will appear in the product creation dropdown.</p>
      </div>
